...
const Hero = ({ setActiveTab, onOpenInfo }) => (
  <div className="relative bg-zinc-950 pt-20 pb-24 border-b border-zinc-900 overflow-hidden min-h-[600px] flex flex-col justify-center">
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-[10%] left-[20%] w-[600px] h-[600px] bg-emerald-500/10 rounded-full blur-[120px] animate-[pulse_8s_ease-in-out_infinite]"></div>
        <div className="absolute bottom-[10%] right-[20%] w-[700px] h-[700px] bg-blue-600/10 rounded-full blur-[140px] animate-[pulse_10s_ease-in-out_infinite_reverse]"></div>
        <div className="absolute inset-0 opacity-[0.03] bg-[url('https://grainy-gradients.vercel.app/noise.svg')]"></div>
    </div>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
      <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-emerald-500/20 bg-emerald-900/10 backdrop-blur-md mb-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        <span className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.8)] animate-pulse"></span>
        <span className="text-xs font-mono text-emerald-100 uppercase tracking-[0.2em]">System Online • Cebu Node</span>
      </div>
      <h1 className="text-6xl md:text-8xl font-bold tracking-tighter text-white mb-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 drop-shadow-[0_0_35px_rgba(255,255,255,0.15)]">TRUST <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-200 via-white to-blue-200">REDEFINED</span></h1>
      <p className="max-w-2xl mx-auto text-lg md:text-xl text-zinc-400 mb-12 leading-relaxed animate-in fade-in slide-in-from-bottom-10 duration-1000 delay-200 font-light">The hardware marketplace powered by <span className="text-white font-medium">Escrow</span> & <span className="text-white font-medium">AI Verification</span>.<br/>Buy GPUs, CPUs, and gear without the risk.</p>
      <div className="flex flex-col sm:flex-row justify-center gap-6 animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-300">
        <button onClick={() => setActiveTab('marketplace')} className="group relative px-10 py-4 bg-white text-black font-bold uppercase tracking-wider overflow-hidden rounded-sm transition-all hover:scale-105 active:scale-95 hover:shadow-[0_0_40px_rgba(255,255,255,0.3)]"><span className="relative z-10 flex items-center gap-2">Enter Market <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform"/></span></button>
        <button onClick={onOpenInfo} className="px-10 py-4 bg-zinc-900/50 border border-zinc-700 text-white font-bold uppercase tracking-wider hover:bg-zinc-800 hover:border-zinc-500 transition-all rounded-sm backdrop-blur-sm">Protocol Specs</button>
      </div>
    </div>
  </div>
);

const Marketplace = ({ tier, trustScore, onStartTrade, onStartChat, onShowSecurity }) => {
    ...
    <div className="min-h-screen bg-zinc-950 pb-20 animate-in fade-in duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 border-b border-zinc-800 pb-6">
          <div className="animate-in slide-in-from-left-4 duration-700"><h1 className="text-4xl font-bold text-white tracking-tighter mb-2">MARKETPLACE</h1><p className="text-zinc-500 font-mono text-sm">REGION: CEBU_CITY // CURRENCY: PHP</p></div>
          <div className="flex gap-4 w-full md:w-auto mt-4 md:mt-0 animate-in slide-in-from-right-4 duration-700"><div className="relative group flex-grow md:flex-grow-0"><input type="text" placeholder="SEARCH_ITEMS..." className="bg-zinc-900 border border-zinc-800 text-white px-4 py-2 w-full md:w-64 focus:outline-none focus:border-emerald-500 font-mono text-sm placeholder-zinc-600 transition-colors" /><Search className="absolute right-3 top-2.5 text-zinc-600 w-4 h-4" /></div></div>
        </div>
        <div className="flex gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide">{['All', 'GPU', 'CPU', 'Peripherals'].map(cat => (<button key={cat} onClick={() => setFilter(cat)} className={`px-4 py-1.5 text-xs font-bold uppercase tracking-wider border transition-all whitespace-nowrap active:scale-95 duration-200 ${filter === cat ? 'bg-emerald-500 border-emerald-500 text-black' : 'bg-transparent border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-500'}`}>{cat}</button>))}</div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{filteredListings.map((item, index) => (<div key={item.id} className="group bg-zinc-900 border border-zinc-800 hover:border-emerald-500/50 transition-all duration-300 flex flex-col hover:-translate-y-2 hover:shadow-xl hover:shadow-emerald-500/10 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${index * 100}ms`, animationFillMode: 'backwards' }}><div className="absolute top-2 left-2 z-10 flex flex-col gap-1">{item.sellerTier === 'official' && (<div className="bg-purple-500 text-white text-[10px] font-bold px-2 py-0.5 uppercase tracking-wider shadow-lg flex items-center gap-1 animate-pulse"><Shield size={10} fill="currentColor"/> Verto Official</div>)}{item.isAiCurated && item.sellerTier !== 'official' && (<div className="bg-emerald-500 text-black text-[10px] font-bold px-2 py-0.5 uppercase tracking-wider shadow-lg flex items-center gap-1"><Zap size={10} /> Verto Select</div>)}</div><div className="relative h-48 overflow-hidden bg-zinc-800"><img src={item.image} alt={item.title} className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700 ease-out opacity-90 group-hover:opacity-100" /></div><div className="p-4 flex flex-col flex-grow"><div className="flex justify-between items-start mb-2"><Badge tier={item.sellerTier} onClick={onShowSecurity} /><span className="font-mono text-emerald-400 font-bold text-sm">{formatCurrency(item.price)}</span></div><h3 className="text-white font-bold mb-1 truncate group-hover:text-emerald-400 transition-colors">{item.title}</h3><p className="text-zinc-500 text-xs mb-3 line-clamp-1">{item.description}</p><p className="text-zinc-600 text-[10px] mb-4 uppercase flex items-center gap-1"><MapPin size={10}/> {item.location}</p><div className="mt-auto pt-4 border-t border-zinc-800 flex gap-2"><button onClick={() => onStartTrade(item)} className="flex-1 bg-zinc-100 hover:bg-emerald-400 text-black text-xs font-bold uppercase py-2 transition-all active:scale-95 flex items-center justify-center gap-1">Buy <Lock size={10} /></button><button onClick={() => onStartChat(item)} className="flex-1 border border-zinc-700 text-zinc-400 hover:text-white hover:border-white text-xs font-bold uppercase py-2 transition-all active:scale-95 flex items-center justify-center gap-1">Chat <MessageCircle size={10} /></button></div></div></div>))}</div>
      </div>
    </div>
  );
};

const Profile = ({ userTier, trustScore, onOpenVerify, onOpenDispute, onShowSecurity }) => (
  <div className="min-h-screen bg-zinc-950 pt-12 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <div className="max-w-4xl mx-auto px-4">
      <div className="relative bg-zinc-900 border border-zinc-800 p-8 mb-8 overflow-hidden group hover:border-emerald-500/30 transition-all duration-500">
        <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity duration-500 transform group-hover:scale-110"><Shield size={120} /></div>
        <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
          <div className="w-24 h-24 bg-zinc-800 rounded-full flex items-center justify-center border-2 border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.1)]"><User size={40} className="text-zinc-400" /></div>
          <div className="text-center md:text-left"><h1 className="text-2xl font-bold text-white mb-1">Guest User</h1><div className="flex items-center justify-center md:justify-start gap-2 mb-4"><span className="text-zinc-500 text-sm font-mono">ID: 99-2810-X</span><Badge tier={userTier} onClick={onShowSecurity} /></div><button onClick={onOpenVerify} className="text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/50 px-4 py-2 uppercase font-bold tracking-wider hover:bg-emerald-500 hover:text-black active:scale-95 transition-all">{userTier === 'unverified' ? 'Start Verification' : 'Upgrade Tier'}</button></div>
          <div className="ml-auto bg-zinc-950 p-6 border border-zinc-800 text-center min-w-[200px] hover:border-emerald-500/50 transition-colors duration-300"><p className="text-zinc-500 text-xs uppercase tracking-widest mb-1">Trust Score</p><p className="text-5xl font-mono font-bold text-emerald-500 animate-in zoom-in duration-500">{trustScore}</p><p className="text-zinc-600 text-[10px] mt-2">Top 15% of Buyers</p></div>
        </div>
      </div>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-zinc-900 border border-zinc-800 p-6 hover:-translate-y-1 transition-transform duration-300"><h3 className="text-white font-bold mb-4 flex items-center gap-2"><Lock size={18} className="text-emerald-500"/> Active Escrows</h3><div className="text-center py-8 border border-dashed border-zinc-800 rounded"><p className="text-zinc-500 text-sm">No active transactions.</p><button className="text-emerald-500 text-xs font-bold uppercase mt-2 hover:underline">Start Shopping</button></div></div>
        <div className="bg-zinc-900 border border-zinc-800 p-6 hover:-translate-y-1 transition-transform duration-300"><h3 className="text-white font-bold mb-4 flex items-center gap-2"><HelpCircle size={18} className="text-emerald-500"/> Support</h3><p className="text-zinc-400 text-sm mb-6">Need help with a meetup? Submit a ticket for assistance.</p><button onClick={onOpenDispute} className="w-full border border-zinc-700 hover:bg-zinc-800 text-white text-xs font-bold uppercase py-3 transition-all active:scale-95 flex items-center justify-center gap-2"><MessageCircle size={16} /> Submit Ticket</button></div>
      </div>
    </div>
  </div>
);
...
const App = () => {
  ...
  return (
    <div className="bg-zinc-950 min-h-screen text-zinc-200 font-sans selection:bg-emerald-500 selection:text-black flex flex-col">
      <nav className="sticky top-0 z-50 w-full bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer group" onClick={() => setActiveTab('home')}>
             <div className="w-8 h-8 bg-emerald-500 flex items-center justify-center transform rotate-45 group-hover:rotate-90 transition-transform duration-500"><span className="transform -rotate-45 group-hover:-rotate-90 transition-transform duration-500 font-bold text-black">V</span></div>
             <span className="font-bold tracking-tighter text-white group-hover:text-emerald-400 transition-colors">VERTO<span className="text-zinc-600 group-hover:text-zinc-400">EXCHANGE</span></span>
          </div>
          <div className="flex items-center gap-6">
             ...
             {/* Settings Icon */}
             <button onClick={() => setShowSettings(true)} className="text-zinc-500 hover:text-white transition-colors">
               <Settings size={20} />
             </button>
      ...
      <main className="flex-grow">
        {activeTab === 'home' && <Hero setActiveTab={setActiveTab} onOpenInfo={() => setShowInfo(true)} />}
